...
import { tauriCommands, type ClassCategory, type RuntimeDiagnostics } from "../lib/tauri";
import type { ClassDetectionResult } from "../types/app-config";
...
export function RuntimeDiagnosticsPage({ classCategories, onError }: RuntimeDiagnosticsPageProps) {
    ...
    <section className="h-full min-w-0 overflow-y-auto px-7 py-6">
      ...
      {diagnostics ? (
        <div className="grid gap-4 pb-6">
          <div className="grid grid-cols-3 gap-4">
            ...
            <StatusCard
              items={[
                ...
                ["识别状态", detectionReasonLabel(diagnostics.detection.lastResult?.reason ?? null)],
                [
                  "识别结果",
                  detectionResultLabel(diagnostics.detection.lastResult, classCategories),
                ],
              ]}
              title="职业识别"
            />
          ...
          <StatusCard
            items={[
              ["设置", diagnostics.activeConfig.autoRunEnabled ? "已启用" : "已关闭"],
...
function detectionResultLabel(
  result: ClassDetectionResult | null,
  classCategories: ClassCategory[],
): string {
  if (result?.classIndex === null || result === null) return "-";

  const classInfo = classCategories
    .flatMap((category) => category.classes)
    .find((item) => item.detectionIndex === result.classIndex);
  return classInfo ? `#${result.classIndex} ${classInfo.name}` : `#${result.classIndex}`;
}
