...
export function ConfigManagementPage({
  ...
  const deletingCustomConfig = deletingCustomConfigId
    ? profiles.customConfigs[deletingCustomConfigId]
    : null;
          ...
          <section className="grid min-h-0 grid-rows-[auto_1fr] rounded border border-slate-200 bg-white shadow-sm">
            ...
            <div className="min-h-0 overflow-y-auto p-4">
              ...
              <div className="mt-3 space-y-2">
                {Object.entries(profiles.customConfigs).map(([configId, customConfig]) => (
                  <div
                    key={configId}
                ...
                {Object.keys(profiles.customConfigs).length === 0 && (
              ...
              <div className="space-y-2">
                {classCategories.map((category) => {
                  ...
                  const configuredCount = category.classes.filter((classInfo) =>
                    Boolean(profileDisplay.classStates[classInfo.id]?.configured),
                  ).length;
                  const hiddenCount = category.classes.filter(
                    (classInfo) => profileDisplay.classStates[classInfo.id]?.hidden ?? false,
                  ).length;
                      ...
                      {!collapsed && (
                        <div className="divide-y divide-slate-100">
                          {category.classes.map((classInfo) => {
                            const state = profileDisplay.classStates[classInfo.id];
