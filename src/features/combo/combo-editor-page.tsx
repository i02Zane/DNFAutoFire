...
import { type RecordingTarget, useComboDraft } from "./use-combo-draft";
...
export function ComboEditorPage({
  ...
  const visibleClassCategories = useMemo(
    ...
    [normalizedClassSearch, profileDisplay.visibleClassCategories],
  );
  const visibleCustomConfigs = useMemo(
    () =>
      Object.entries(profiles.customConfigs).filter(([, customConfig]) => {
        const name = customConfig.name.trim() || "未命名配置";
        return !normalizedClassSearch || name.toLowerCase().includes(normalizedClassSearch);
      }),
    [normalizedClassSearch, profiles.customConfigs],
  );

  function deleteCombo(comboId: string) {
    deleteComboDraft(comboId);
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {visibleCustomConfigs.map(([configId, customConfig]) => {
                  ...
                  const configured =
                    profileDisplay.customConfigStates[configId]?.hasCombos ?? false;
                  return (
                    <button
                      key={configId}
            ...
            <div key={category.name} className="grid grid-cols-[84px_1fr] items-center gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {category.classes.map((classInfo) => {
                  ...
                  const configured = profileDisplay.classStates[classInfo.id]?.hasCombos ?? false;
                  return (
                    <button
                      key={classInfo.id}
                      className={classButtonClass(active)}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <h2 className="mt-1 truncate text-xl font-semibold text-slate-900">
                    {getConfigDisplayName(activeConfigId, profileDisplay)}
                  </h2>
                </div>
                <div className="flex items-center gap-2">
                  <button
    ...
    <div className="flex min-w-0 flex-wrap items-start gap-2">
      ...
      <div className="w-[132px]">
        <NumberField
          ...
          min={10}
          value={action.holdMs}
          onChange={(value) => onUpdate({ holdMs: value })}
        />
      </div>
    </div>
  );
      ...
      <div className="flex flex-wrap gap-2">
        <div className="w-[132px]">
          <NumberField
            ...
            onChange={(value) => onUpdate({ keyHoldMs: value })}
          />
        </div>
        <div className="w-[148px]">
          <NumberField
            ...
            onChange={(value) => onUpdate({ keyGapMs: value })}
          />
        </div>
      </div>
      ...
      >
        ...
        <input
          ...
          onBlur={(event) => {
            ...
            const normalizedValue = toU16Integer(nextValue, value, min);
