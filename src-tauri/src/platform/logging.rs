...
use crate::config::{read_log_level_setting, settings_config_path, LogLevelSetting};
...
pub(crate) fn initialize() -> LoggingState {
    ...
    let log_level = read_log_level_setting(&settings_config_path());
...
pub(crate) fn update_log_level(log_level: LogLevelSetting) -> AppResult<()> {
    let filter = build_filter_for_level(log_level);
    let handle = FILTER_HANDLE
        .get()
        .ok_or_else(|| AppError::logging("日志系统尚未初始化"))?;
    handle
        .modify(|current| {
            *current = filter;
        })
        .map_err(|error| AppError::logging(format!("更新日志等级失败: {error}")))
}
...
fn initialize_file_subscriber(log_file_path: &Path, log_level: LogLevelSetting) -> AppResult<()> {
    let file = OpenOptions::new()
        .create(true)
        .append(true)
        .open(log_file_path)
        .map_err(|error| AppError::logging(format!("打开日志文件失败: {error}")))?;
    ...
    #[cfg(debug_assertions)]
    {
        ...
        tracing_subscriber::registry()
            ...
            .try_init()
            .map_err(|error| AppError::logging(format!("注册日志订阅器失败: {error}")))?;
    }
