...
use crate::platform::logging::format_hotkey;
...
pub(crate) fn validate_hotkey(hotkey: &Hotkey) -> AppResult<()> {
    if !(hotkey.ctrl || hotkey.alt || hotkey.shift) || is_modifier_vk(hotkey.vk) {
        return Err(AppError::validation(
            "启动/停止快捷键必须是组合键，例如 Ctrl + F8。",
        ));
    }
    Ok(())
}
...
pub(crate) fn register_windows_hotkey(
    ...
    let handle = thread::spawn(move || {
        ...
        if let Err(error) = registered {
            ...
            let _ = ready_tx.send(Err(AppError::hotkey(format!(
                "注册启动/停止快捷键失败: {error}"
            ))));
            return;
        }
        ...
        while unsafe { GetMessageW(&mut msg, None, 0, 0) }.as_bool() {
            if msg.message == WM_HOTKEY && msg.wParam == WPARAM(TOGGLE_HOTKEY_ID as usize) {
                ...
                if let Err(error) = runtime_supervisor.toggle_assistant(&app_handle) {
                    tracing::warn!(error = %error, "全局快捷键切换助手状态失败");
                    show_error_message_box(&error.message);
                }
                continue;
            }
            unsafe {
